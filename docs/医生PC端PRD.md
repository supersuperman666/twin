...
- 家庭医生或专科医生均可作为患者当前主责医生；当前主责医生负责当前管理方案、预警、随访和医生建议闭环。
...
扫码绑定用于建立医生与患者的服务关系。当前版本以“一个患者当前只有一个主责医生”为核心模型，主责医生可以是家庭医生，也可以是专科医生。当前版本只支持“医生生成二维码，患者扫码确认授权”这一条主流程；暂不纳入后台/机构分配患者、患者扫码机构码、服务团队绑定。绑定生效后，医生端可查看患者授权范围内的数据，患者端可看到当前主责医生、医生建议、随访计划和方案待办。
...
| 关系类型 | 说明 | 主要权限 |
| --- | --- | --- |
| 主责医生 | 患者当前唯一主责医生，可以是家庭医生或专科医生 | 创建/调整管理方案、处理预警、创建随访、下发医生建议、查看授权数据 |
...
- 家庭医生和专科医生都可以成为主责医生。
- 当前管理方案、预警处理、随访计划、医生建议默认归属主责医生。
- 转诊不自动变更主责医生；转诊先作为医生建议/转诊记录存在。
...
| 类型 | 是否改变主责医生 | 说明 |
| --- | --- | --- |
| 转诊建议/专科评估 | 否 | 主责医生不变，患者按医生建议线下就诊或绑定协作医生，结果回流后由主责医生复盘 |
...
P0 不单独建设“工作台”。工作台能力合并到“患者管理首页”的顶部统计卡、快捷筛选和待处理事项字段中，避免与患者管理列表形成重复入口。

| 一级模块 | 页面 | 说明 |
| --- | --- | --- |
| 患者管理 | 患者列表、顶部快捷统计、筛选、扫码绑定、患者详情入口 | 医生进入系统后的默认首页，承担轻量工作台能力 |
| 预警中心 | 预警列表、预警详情、处理抽屉、处置闭环 | 按异常事件视角集中处理预警 |
| 方案管理 | 待确认方案、执行中方案、方案详情、方案确认/调整 | 按方案状态视角集中处理系统草稿和患者方案 |
| 随访计划 | 随访列表、随访详情、完成随访、改期/取消 | 按时间任务视角管理待随访和逾期随访 |

患者详情不是一级菜单，而是所有一级模块都可进入的单患者深度分析页面。

### 6.2 患者详情核心模块

| 患者详情模块 | 说明 |
| --- | --- |
| 总览 | 30 秒内判断患者当前状态、待处理事项、当前方案和下次随访 |
| 健康档案 | 查看患者长期健康背景、疾病史、治疗史、生活方式、家族史和设备信息 |
| 健康筛查 | 查看筛查结果、风险因子、量表明细和医生确认疾病过程 |
| 数据分析 | 医生专业分析，包括临床摘要、疾病专题、多指标关联和证据链 |
| 预警记录 | 查看该患者全部预警、处理状态和处理结论 |
| 管理方案 | 查看当前生效方案、待确认草稿、历史方案和调整记录 |
| 随访记录 | 查看、创建、完成、改期或取消该患者随访 |
| 时间轴 | 沉淀档案、筛查、数据、预警、方案、随访等关键事件 |

### 6.3 后续扩展模块

以下能力不作为 P0 一级导航，可在患者详情或后台配置中逐步扩展：

- 数字孪生画像：个体基线、趋势偏离、风险解释。
- 设备与报告：设备状态、睡眠报告、血氧报告。
- 医生建议：复测建议、就医/转诊建议、健康指导、患者阅读、执行反馈。
- 数据看板：医生效率、患者管理效果、模型效果。
- 系统设置：账号、角色、规则、模板、审计。
...
P0 不单独建设“工作台”菜单。医生登录后默认进入“患者管理”。

独立工作台会与患者管理列表高度重叠，容易形成两套相似的统计、筛选和待办队列。P0 阶段优先把资源投入患者详情、专业数据分析、预警处理、方案确认和随访闭环。

### 8.2 合并原则

患者管理首页承担轻量工作台能力，但不做复杂任务中枢。

| 原工作台能力 | P0 承载位置 |
| --- | --- |
| 今日待处理预警 | 患者管理顶部统计卡 + 预警中心 |
| 待确认方案 | 患者管理顶部统计卡 + 方案管理 |
| 今日待随访/逾期随访 | 患者管理顶部统计卡 + 随访计划 |
| 数据缺失患者 | 患者管理顶部统计卡 + 患者列表待处理事项 |
| 重点关注患者 | 患者管理筛选条件 + 患者列表重点标记 |

### 8.3 页面心智

| 页面 | 核心对象 | 回答的问题 | 主操作 |
| --- | --- | --- | --- |
| 患者管理 | 患者 | 我要找哪个患者？哪些患者需要关注？ | 查看详情、标记重点、添加患者 |
| 预警中心 | 预警事件 | 哪些异常事件需要处理？ | 处理预警、创建随访、生成方案草稿 |
| 方案管理 | 管理方案 | 哪些方案需要确认或调整？ | 确认下发、编辑、生成新版本 |
| 随访计划 | 随访任务 | 哪些随访今天要做或已逾期？ | 完成随访、改期、取消 |
| 患者详情 | 单个患者 | 这个患者为什么异常、下一步怎么管理？ | 数据分析、处理预警、调整方案、创建随访 |
...
高级筛选区
  默认收起；入口固定在统计概览最右侧“更多筛选”
  展开后在统计概览下方展示完整筛选面板
  重点关注、风险状态、疾病风险、确诊疾病、待处理事项、方案状态、随访状态、数据状态、医患关系、设备状态
...
- 页面顶部不再重复展示“患者管理”大标题，顶部只保留搜索、添加患者和必要工具。
- 统计卡高度建议控制在 72-84px，卡片宽度使用等分或横向自适应，不做大面积看板。
- 每个统计卡标题右侧展示问号 icon，hover 展示口径说明、点击行为和更新时间。
- “更多筛选”作为统计概览最右侧功能卡，不计入统计指标；卡片展示当前已选高级筛选数量。
- 筛选条件默认收起，避免占用列表横向空间；点击“更多筛选”后在统计概览下方展开完整筛选面板。
- 高级筛选展开时不遮挡列表，不使用左侧常驻筛选栏，保证患者列表字段宽度和阅读效率。
- 患者列表行高建议控制在 76-88px，保证一屏能看到更多患者。
- 患者列表首列固定展示患者姓名、性别年龄、手机号脱敏、患者 ID 和星标。
- 鼠标悬浮患者行时，行背景轻微高亮，右侧操作区显性展示。
...
| 重点关注 | 医生主动标记重点关注的患者 | 筛选重点关注 |
| 疾病风险待确认 | 健康筛查提示疾病风险，但医生尚未确认或驳回的患者 | 筛选待确认疾病风险患者 |
| 待处理预警 | 存在待医生处理预警的患者 | 筛选预警患者，也可跳转预警中心 |
| 待确认方案 | 系统已为患者生成管理方案，但尚未医生审核确认 | 筛选待确认方案患者 |
| 今日待随访 | 今天计划随访或已逾期未处理的患者 | 筛选今日/逾期随访患者 |
| 数据缺失 | 连续缺失关键记录或设备未同步患者 | 筛选数据缺失患者 |
| 更多筛选 | 当前已选择的高级筛选条件数量 | 展开/收起完整筛选面板 |

统计卡交互：

- 点击统计卡即应用快捷筛选；再次点击同一卡片取消快捷筛选。
- 选中卡片使用蓝色描边和浅蓝底，不改变页面结构。
- 点击“更多筛选”只控制高级筛选面板展开/收起，不改变患者结果。
- 统计卡右上角问号 icon hover 展示说明浮层：
  - 统计口径。
  - 数据更新时间。
  - 点击后的筛选条件。
- 统计卡不展示长说明，说明信息全部进入 hover 浮层。
- 移动鼠标离开浮层后自动关闭。
- 若统计数量为 0，卡片仍展示，但点击后进入空结果页。
...
| 重点关注 | 全部、仅看重点关注 |
| 风险状态 | 稳定、需关注、需干预 |
| 疾病风险 | 全部、糖尿病风险、慢阻肺风险、睡眠呼吸暂停风险、高血压风险、待确认 |
| 确诊疾病 | 糖尿病、慢阻肺、睡眠呼吸暂停、高血压、多病共管 |
| 待处理事项 | 全部、有预警、待确认方案、今日待随访、数据缺失 |
| 方案状态 | 无方案、草稿、已下发待患者确认、执行中、已驳回、已停用、已完成 |
| 方案提醒标签 | 全部、患者未确认、患者有疑问、患者无法执行、即将到期、待复盘 |
| 随访状态 | 无随访、今日待随访、逾期随访、近期已随访 |
| 数据状态 | 正常、关键指标缺失、设备未同步、记录完整率低 |
| 医患关系 | 全部、主责医生患者、协作医生患者 |
| 设备状态 | 全部、关键设备已绑定、关键设备未绑定、设备同步异常 |

筛选规则：

- 支持多条件组合。
- 顶部统计概览卡片支持点击快捷筛选。
- 高级筛选默认收起，通过统计概览最右侧“更多筛选”展开；展开后支持手动组合筛选。
- 点击“全部患者”清空快捷筛选和手动筛选。
- 第一版不支持保存常用筛选。
- 搜索框与筛选条件联动，搜索支持患者姓名、患者 ID、患者手机号。
- 搜索输入后实时过滤列表；按 Enter 后固定搜索条件。
- 搜索框支持一键清空。
- 已选筛选条件在列表上方以条件标签展示，支持单个删除。
- 高级筛选收起时，已选条件仍在列表上方展示，避免医生不知道当前处于筛选态。
- “更多筛选”卡片展示已选高级筛选数量，方便医生判断是否存在隐藏筛选条件。
- 当筛选结果为空时，展示空状态并提供“清空筛选”按钮。

### 9.5 全局分页

列表型页面必须统一使用分页组件，覆盖患者管理、预警中心、方案管理、随访计划等主列表页面。

分页组件展示规则：

- 展示当前范围和总数，例如“第 1-20 条/总共 4123 条”。
- 支持上一页、下一页。
- 支持页码快捷跳转；页码过多时使用省略号折叠。
- 支持每页条数选择：10 条/页、20 条/页、50 条/页、100 条/页，默认 20 条/页。
- 支持输入页码后按 Enter 跳转。
- 筛选、搜索、切换统计卡后，分页自动回到第 1 页。
- 分页组件固定放在当前列表底部，不放在页面顶部，避免干扰医生先看数据。

### 9.6 排序规则

患者列表默认按绑定时间倒序排列，最新绑定的患者排在前面。

第一版不支持复杂排序和医生自定义排序。医生通过顶部统计概览卡片和高级筛选区快速定位高风险、今日随访、待确认方案、数据缺失等患者。
...
| 方案状态 | 无方案、草稿、已下发待患者确认、执行中、已驳回、已停用、已完成；患者有疑问、无法执行、待复盘等作为提醒标签展示 |
...
- 疾病标签分两层展示：
  - 医生确认疾病使用蓝色标签。
  - 疾病风险待确认使用橙色描边标签，并显示“待确认”。
- 重点关注不进入操作栏，只通过患者信息区星标操作。
- 列表默认不展示过多文本说明，长内容 hover 展示完整内容。
- 最新关键指标只展示 1-3 个最关键指标，优先展示异常指标。
- 待处理事项使用标签聚合展示，例如“1 个预警”“待确认方案”“今日随访”“数据缺失”。

### 9.8 行内快捷操作

患者管理列表的定位是“找患者、筛患者、进入详情”，不在行内承载复杂医疗处置。复杂动作进入患者详情、预警中心、方案管理或随访计划完成，避免列表操作过重。

| 操作 | 触发条件 | 结果 |
| --- | --- | --- |
| 查看详情 | 所有患者 | 进入患者详情页，作为行内主操作 |
| 更多 | 有待处理事项或特殊状态 | 展开次级菜单 |

“更多”菜单 P0 仅保留必要入口：

| 操作 | 触发条件 | 结果 |
| --- | --- | --- |
| 去处理预警 | 有待处理预警 | 跳转预警中心并定位该患者/预警 |
| 去确认方案 | 有方案草稿待审核 | 跳转方案管理并定位该患者/方案 |
| 去完成随访 | 今日待随访或逾期随访 | 跳转随访计划并定位该患者/随访 |
| 去确认疾病 | 有疾病风险待确认 | 进入患者详情健康筛查 Tab 并定位待确认疾病 |
| 查看数据缺失 | 关键指标缺失或设备同步异常 | 进入患者详情数据分析/健康档案设备区域 |

以下动作不放在患者列表行内：

- 发送医生建议：进入患者详情或预警/随访处理流程后发起。
- 创建随访：进入患者详情或预警处理流程后创建。
- 调整方案：进入患者详情的管理方案 Tab 或方案管理详情完成。
- 绑定设备提醒：进入患者详情的健康档案/设备信息区域处理。
...
- “更多”仅在患者存在待处理事项、疾病风险待确认或数据缺失时展示。
- 更多菜单最多展示 4 个动作，按紧急程度排序：处理预警 > 确认疾病 > 确认方案 > 完成随访 > 查看数据缺失。
- 没有待处理事项时不展示“更多”，避免操作噪声。

### 9.8.1 列表页进一步优化建议

- 患者列表建议默认使用表格，不使用大卡片；医生需要高密度扫描。
- 顶部统计卡作为快捷筛选，不做复杂大看板。
- 筛选面板支持收起，避免长期占用横向空间。
- 搜索框作为第一优先级入口，适合门诊或电话随访场景快速定位患者。
- 疾病风险和确诊疾病必须同时展示，避免医生错把筛查风险当成确诊疾病。
- 待处理事项不应散落在多个字段里，应聚合成统一标签区。
- 操作栏控制在 1 个主按钮 + 1 个更多菜单，避免行内动作过多。
- 如果未来患者量增大，可增加“密度切换”：舒适 / 紧凑；P0 默认紧凑。
...
| 方案执行 | 当前方案目标、执行率、调整提醒 |
| 医生建议 | 最近医生建议、患者是否已读/执行 |
...
P0 采用一个通用“患者管理事件时间轴”组件，不单独建设多套时间轴。不同页面通过默认筛选展示不同事件：

| 使用位置 | 默认筛选 | 用途 |
| --- | --- | --- |
| 患者 360 总览 | 全部关键事件 | 看患者完整管理轨迹 |
| 患者 360 - 管理方案 Tab | 方案事件 | 看方案创建、下发、确认、调整、停用和版本变化 |
| 患者 360 - 随访 Tab | 随访事件 | 看随访创建、准备、完成、结论和后续动作 |
| 预警详情 | 当前预警相关事件 | 看预警触发、复测、处理、随访和方案调整 |
...
| --- | --- |
| 事件时间 | 事件发生时间，按倒序展示 |
| 事件类型 | 筛查、记录、设备、预警、方案、随访、医生建议 |
| 事件标题 | 如“糖尿病中风险方案已下发”“计划内随访已完成” |
| 来源对象 | 关联方案、随访、预警、任务、报告或记录 |
| 操作者 | 系统、患者、医生、设备 |
| 关键摘要 | 事件核心结果，例如方案版本、随访结论、预警等级 |
| 后续动作 | 生成新方案、创建随访、发送医生建议、补充任务等 |
| 可见范围 | 医生可见/患者可见/医生内部 |

视觉与交互：

- 时间轴只读展示，P0 不支持在时间轴内直接编辑事件。
- 左侧为时间线，右侧为事件卡片；重点事件使用标签色区分。
- 支持事件类型筛选：全部、管理方案、随访、预警、医生建议、设备/报告、关键记录。
- 点击事件卡片打开详情抽屉，展示关联对象和原始数据入口。
- 方案、随访、预警等关键医疗事件必须展示操作者和时间。
- 被医生标记无效的记录不从时间轴删除，而是展示“已标记无效”状态。
...
| 事件 | 说明 |
| --- | --- |
| 草稿生成 | 系统初始化、医生创建、模板创建、随访/预警后调整 |
| 医生确认下发 | 展示确认医生、方案版本、适用疾病、计划生效时间 |
| 患者确认执行 | 展示患者确认时间和反馈状态 |
| 方案执行中 | 展示关键目标、执行率、异常摘要 |
| 随访触发调整 | 展示来源随访、医生结论和原方案调整记录 |
| 预警触发调整 | 展示来源预警、处理结论和方案调整动作 |
| 方案停用/完成 | 展示停用原因、结束时间和后续方案 |

交互：

- 管理方案 Tab 默认展示当前方案卡片 + 方案版本列表 + 方案管理时间轴。
- 点击时间轴事件可打开方案版本详情、患者确认记录、随访结论或预警处理详情。
- 方案版本对比 P0 只展示关键变更摘要，不做复杂逐字段 diff。
...
| --- | --- |
| 随访创建 | 展示来源、计划时间、准备材料 |
| 患者准备 | 展示准备完成度、缺失材料、患者反馈 |
| 随访改期/取消 | 展示操作人、原因、原计划时间和新计划时间 |
| 随访完成 | 展示医生结论、患者端建议、内部备注入口 |
| 生成补充任务 | 展示补充记录、报告上传或评估量表任务 |
| 触发方案调整 | 展示原方案调整记录、来源随访和医生结论 |
| 创建下次随访 | 展示下次随访时间和准备材料 |

交互：

- 随访 Tab 默认展示当前随访计划 + 历史随访结论 + 随访时间轴。
- 点击时间轴事件打开随访详情抽屉或随访记录详情。
- P0 只做只读时间轴，不做拖拽改期或排班日历。
...
- 不做复杂 AI 预测预警，P0 以规则预警为主。
- 不做自定义预警规则后台，先内置核心规则。
- 不做预警大屏、质控看板和批量处理预警。
- 不做预警分派、转交和多医生协同处理。
- 不做预警自动诊断、自动调整管理方案或自动下发方案。
- 不对接医保、院内急诊系统或院内工单系统。
- 不引入过多预警等级，P0 保留提醒、重要、紧急。
...
| 预警 | 默认触发规则 | 等级 | 患者端动作 | 医生端动作 |
| --- | --- | --- | --- | --- |
| 关键用药未完成 | 关键用药提醒连续 2 次未完成 | 重要 | 补充原因 | 发送建议或创建随访 |
...
| 调整管理方案 | 是 | 仅在长期方案确需变化时使用，需填写调整类型与原因 |
| 创建随访 | 是 | 患者端展示随访计划 |
...
- P0 必须保留预警到复测、随访、医生建议、临时干预，以及必要时管理方案调整入口的闭环。
...
为医生远程慢病管理复盘提供摘要，不提供在线问诊、视频问诊或 IM 聊天能力：
...
| 模块 | 定位 | 产物 |
| --- | --- | --- |
| 初始化方案模板 | 系统根据疾病和风险分层生成管理建议草稿 | 待医生审核的草稿 |
| 患者管理方案 | 医生确认后的正式阶段计划 | 患者端待办、目标、预警规则、随访计划 |
| 随访计划 | 检查方案执行效果的时间安排 | 计划内随访、临时随访、预警后随访 |
| 医生建议 | 单次医疗处置要求、复测要求、复诊/转诊要求或健康管理指导 | 消息卡片、预警处理意见、随访后医生建议 |
...
- 方案主记录不因普通调整而新建；调整时原方案进入调整态，版本号递增。
- 调整编辑期间，患者端继续执行上一已生效版本任务，避免监测、用药或随访中断。
- 患者确认新版本后，系统自动结束旧版本未完成任务，并按新版本配置生成新的患者端任务；历史已完成任务和执行记录不回写、不重算。
- 系统可自动生成管理方案草稿，但不得自动下发为患者执行方案；所有方案必须由主责医生审核确认后，才能推送至患者端。

P0 不做范围：

- 不做复杂方案模板后台，方案管理作为 P0 一级菜单仅承载待确认方案、执行中方案和方案详情闭环。
- 不做模板后台管理，P0 先内置模板配置。
- 不做复杂规则引擎，P0 采用模板配置和少量规则字段。
- 不做方案多医生共同编辑。
- 不做患者选择执行哪个医生方案。
- 不做复杂方案版本逐字段 diff，P0 只展示关键变更摘要。
- 不做方案批量下发。
- 不做方案自动下发，必须医生确认。
- 不做在线开药、处方流转、药品支付或医保结算。
- 不做复杂生活方式课程体系，P0 只做轻量指导和待办。
- 不允许患者端修改方案目标、频率、用药剂量或治疗执行要求。
- 不做量化目标自定义创建，P0 仅支持启用/停用/调整预置目标值。
- 不做量化目标关联方案项映射，P0 目标独立配置，不自动关联测量/用药/设备方案。
- 不做左侧证据区独立栏，P0 证据信息下沉到各模块卡片摘要区。
- 不做综合方案冲突、目标冲突、测量频率冲突、用药提醒冲突、随访冲突检测，P0 仅做疾病域冲突。
- 不做"合并方案"和"仅新增疾病域"冲突处理选项，P0 仅支持替换或取消下发。
- 不把患者疑问、无法执行、调整需求、复盘提醒做成独立方案状态，统一作为提醒标签、事件或操作入口处理。
- 不做模块间自动联动提示（测量→目标、用药→目标、设备→目标），P0 各模块独立编辑。
...
flowchart TD
  ...
  E -->|缺少必填项| D
  E -->|疾病域冲突| F["选择：替换旧方案或取消下发"]
  F --> E
  E -->|校验通过| G["医生确认下发"]

  G --> H["已下发待患者确认"]
  H --> I{"患者反馈"}
  I -->|已知晓| J["执行中，旧任务换代并生成新版待办"]
  I -->|有疑问或无法执行| J2["执行中（标注疑问/无法执行标签），通知医生"]

  J --> K["执行数据回流"]
  K --> L{"是否需要调整"}
  L -->|继续执行| J
  L -->|调整方案| C2["原方案进入调整态，回到方案编辑器"]
  L -->|停用/完成| M["已停用/已完成"]
```
...
| 场景 | 触发条件 | 方案状态 |
| --- | --- | --- |
| 首次建档 | 患者完成筛查并与医生绑定 | 草稿（系统推荐） |
| 医生确认疾病 | 疾病风险标签被医生确认 | 草稿（系统推荐） |
| 风险等级变化 | 健康风险分跨等级下降或出现紧急预警 | 草稿（系统推荐） |
...
- 若患者已有已下发或执行中方案，系统不得直接覆盖患者正在执行的内容；首次建档、疾病确认或风险变化只更新同疾病域未下发草稿，医生确认后再下发。
- 管理方案定位为长期方案，不因单次预警或单次随访频繁进入调整态。
- 预警后、随访后如需短期补充观察或额外跟进，优先创建“临时干预”，支持临时随访和临时任务。
- 只有医生明确判断长期策略需要变化时，当前执行方案才进入调整态；进入前必须填写调整类型和调整原因。
- 当前执行方案进入调整态时，方案版本号递增，旧版本任务在患者确认新版前继续有效。
- 患者确认新版方案后，旧版本未完成任务自动关闭，关闭原因记录为“方案版本调整”；新版任务按当前配置重新生成。
- 已完成任务、设备同步结果、手动记录和随访证据保留在原版本，不随新版配置重算依从性。
...
- 必须模块不展示启用/停用开关，只展示完成状态和编辑入口，避免医生误解必填模块可以关闭。
- 条件模块才展示“纳入本方案”开关；未纳入时不校验该模块内部字段。
- 条件模块若被系统判定为不可关闭，开关置灰并展示原因，例如“当前 AHI 目标依赖睡眠设备数据，需保留设备监测方案”。
- 条件模块若为系统强推荐但允许关闭，关闭时需选择原因并留痕。
- 通用字段保持一致：适用疾病、执行频率、开始/结束时间、医生内部备注。
- 患者端说明不作为医生填写字段，系统需根据管理目标、执行规则、随访安排和异常处理动作自动编译生成，并提供只读预览。
- 系统编译患者端说明时必须使用患者可理解语言，避免专业术语和规则配置语言，例如将 SpO2、AHI、CPAP 转换为血氧、睡眠呼吸事件、呼吸机/通气设备等表达。
...
| --- | --- | --- |
| 已完成 | 模块已满足下发校验 | 可继续编辑 |
| 待补充 | 必须模块缺少必填字段，或已纳入模块缺少内部必填字段 | 点击定位缺失项 |
| 建议完善 | 不阻断下发，但系统建议补充，例如生活方式建议为空 | 可忽略或编辑 |
| 未纳入 | 条件模块关闭，不参与本方案 | 展示关闭原因和“重新纳入” |
| 有风险 | 停用强推荐项、关闭建议模块或存在数据依赖冲突 | 点击查看风险原因 |
| 已修改 | 医生修改过系统推荐内容 | 展示修改留痕 |

患者端展示原则：

- 患者端不展示复杂配置，只展示“今日待完成/今日记录/用药提醒/随访准备”。
- 患者端说明必须由系统生成且面向患者可理解，例如“今天晚餐后 2 小时记录血糖”，不展示规则配置语言，医生不可在模块配置中手动填写。
...
| 服用频次 | 是 | 枚举：每日 1 次、每日 2 次、每日 3 次、每日 4 次、每 8 小时 1 次、每 12 小时 1 次、隔日 1 次、每 3 日 1 次、必要时、紧急时 |
| 服用时间 | 是 | 如早餐后、晚餐后、睡前 |
...
- 涉及复杂治疗调整、高风险药物或超出基层诊疗能力的内容，应通过复诊/转诊建议或专科协作处理。
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| 阶段目标 | textarea/template | 是 | 面向本周期的综合管理结果，建议 20-80 字 |
| 目标周期 | select | 是 | 默认跟随方案周期 |
| 量化目标 | target list | 是 | 系统预置，医生可启用/停用/调整目标值；至少启用 1 项 |
...
| 目标值 | number/range | 是 | 预置默认值，医生可调整 |
...
- 统计周期（P0 默认跟随方案周期）
- 是否用于预警辅助（P0 预警独立配置）
- 个体化原因（P0 简化为修改目标值时自动留痕）
- 医生内部备注（移到方案基础信息备注字段）

填写体验（P0 简化）：

- 选择目标项后自动带出单位、默认范围和患者端文案。
- 医生修改默认目标值时自动留痕，不弹表单。
- 管理目标模块始终汇总展示全部已启用目标。

P1 延后填写体验：
- 点击”新增目标”从疾病目标库选择或自定义创建。
- 在指标测量/用药/设备方案中自动提示关联目标。
...
| 适用疾病 | enum/list | 是 | 糖尿病、慢阻肺、睡眠呼吸障碍、高血压 |
| 适用风险等级 | enum | 是 | 低/中/高/危急 |
| 适用人群 | list | 否 | 初诊、已确诊、设备已绑定、依从性差、近期预警等 |
| 方案周期 | number + unit | 是 | 7/14/30/90/180 天 |
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| 指标编码 | enum | 是 | blood_glucose、blood_pressure、spo2、resp_rate、weight、sleep_report |
| 指标名称 | select | 是 | 血糖、血压、SpO2、呼吸频率、体重、睡眠报告 |
| 测量场景 | enum/list | 条件必填 | 血糖需选择凌晨/空腹/早餐后2h/午餐前/午餐后2h/晚餐前/睡前/随机；血压需选择晨起/睡前/随机；血氧需选择静息后/活动后/睡前；呼吸频率需选择静息后/活动后/睡前；支持多选 |
| 测量频率 | enum/object | 是 | 每日、每周 N 天 |
...
| 服用频次 | enum | 否 | 每日 1 次、每日 2 次、每日 3 次、每日 4 次、每 8 小时 1 次、每 12 小时 1 次、隔日 1 次、每 3 日 1 次、必要时、紧急时 |
...
| 患者端说明 | text | 否 | 无默认值，医生需要时填写 |

随访计划配置：

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| 是否安排随访 | boolean | 是 | 默认开启 |
| 首次随访时间 | number + unit | 条件必填 | 安排随访时必填，方案下发后第几天 |
| 随访频率 | enum/object | 条件必填 | 安排随访时必填；每周、每 2 周、每月、按预警触发 |
| 随访方式 | enum/list | 是 | 小程序问卷、电话、线下、医生记录 |
...
| 冲突类型 | 判断规则 | 处理要求 |
| --- | --- | --- |
| 疾病域冲突 | 同一患者、同一疾病域、同一时间存在多个执行中或待患者确认方案 | 必须选择：替换旧方案或取消下发 |
...
| 延后冲突类型 | 说明 |
| --- | --- |
| 综合方案冲突 | 综合多病方案覆盖了已有单病执行中方案的疾病域 |
| 目标冲突 | 同一指标存在不同量化目标范围 |
| 测量频率冲突 | 同一时段重复生成相同测量事项 |
| 用药提醒冲突 | 同一药品同一时间不同剂量/频次提醒 |
| 随访冲突 | 同一周期重复生成多个计划内随访 |

P1 延后下发前处理方式：合并方案、仅新增疾病域、主责医生变更后待复核。
...
- 预警详情：创建临时干预；如确需修改长期方案，可进入调整管理方案。
- 随访详情：随访结束后更新方案。
...
| 模式 | 触发场景 | 默认内容 | 主要动作 |
| --- | --- | --- | --- |
| 系统草稿审核 | 系统根据风险分层生成待确认方案 | 自动带出疾病、管理目标、测量/记录/用药/设备/随访建议 | 确认下发、修改后下发、驳回推荐 |
...
P0 不做左侧证据区独立栏。证据信息（近期指标趋势、症状摘要、设备数据）下沉到各模块卡片的"系统推荐依据"和"最近数据"区域展示。三栏布局改为两栏布局，确保中间编辑区在 1280px 屏幕上有足够可读宽度。

核心交互：

- 医生从 0 创建时，先选择“适用疾病 + 方案周期”，系统再推荐对应目标库和方案模块；医生也可选择空白创建。
- 医生可切换疾病模板，系统结合患者当前风险状态带出测量/记录/用药/设备方案、随访频率和预警规则。
...
- 点击“患者可见内容预览”展示患者小程序中将看到的方案文案、今日待完成事项、随访提醒和异常处理提示；该入口用于医生下发前确认患者是否能理解和执行，不是患者实际页面入口。
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| 方案名称 | input | 是 | 默认按“疾病 + 周期”生成，医生可修改 |
...
| 患者端摘要 | textarea | 否 | 面向患者的 30-120 字简明说明，可由系统根据方案自动生成后医生编辑 |
...
模块校验采用“模块是否纳入 + 模块内部字段”的两层判断：

- 必须模块：默认纳入，不展示启用/停用开关；下发前永远校验内部必填字段。
- 条件模块：展示“纳入本方案”开关；未纳入时不校验内部字段。
- 条件模块已纳入：按该模块字段规则校验。
- 条件模块不可关闭：开关置灰，展示不可关闭原因。
- 条件模块建议启用但医生关闭：允许关闭，但必须选择原因并留痕。

一个管理方案最小可下发结构必须包含以下 6 个模块：
...
| 模块 | 模块类型 | 是否展示开关 | 未纳入是否校验内部字段 | 启用/默认纳入后最低要求 |
| --- | --- | --- | --- | --- |
| 方案基础信息 | 必须模块 | 否 | 不适用 | 方案名称、适用疾病、方案周期、计划开始日期必填 |
| 管理目标 | 必须模块 | 否 | 不适用 | 阶段目标 + 至少 1 个启用量化目标 |
| 指标测量方案 | 必须模块 | 否 | 不适用 | 至少 1 个关键指标测量项；停用核心测量项需填写原因 |
| 预警规则 | 必须模块 | 否 | 不适用 | 至少保留核心安全预警；停用高风险规则需填写原因 |
| 随访计划 | 必须模块 | 否 | 不适用 | 默认安排随访；如不安排，仅关闭随访安排 |
| 患者指导 | 必须模块 | 否 | 不适用 | 系统根据目标、待办、随访和异常动作生成患者可见说明，需使用通俗表达 |
| 症状记录方案 | 条件模块 | 是，“纳入本方案” | 是 | 选择症状项、记录频率和触发条件 |
| 用药方案 | 条件模块 | 是，“纳入本方案” | 是 | 药品名称、剂量、频次、用药时间、用药来源；无用药可不纳入 |
| 设备监测方案 | 条件模块 | 是，“纳入本方案” | 是 | 设备类型；患者端说明可按需填写；依赖设备数据时不可关闭 |
| 生活方式方案 | 可选模块 | 是，“纳入本方案” | 是 | 至少 1 条患者可执行指导；未纳入不阻断下发 |

条件模块启用规则：

| 模块 | 默认建议 | 不可关闭场景 | 关闭原因要求 |
| --- | --- | --- | --- |
| 症状记录方案 | 慢阻肺、睡眠呼吸障碍、糖尿病高风险/低血糖风险建议纳入 | 当前方案将症状作为预警或随访准备材料时不可关闭 | 高风险患者关闭时必填原因 |
| 用药方案 | 患者已有医生确认用药时建议纳入 | 启用了用药执行率目标，或方案明确包含用药提醒时不可关闭 | 患者已有用药但关闭时必填原因 |
| 设备监测方案 | 存在设备、睡眠报告、CPAP 或血氧连续监测时建议纳入 | 启用 AHI、ODI、睡眠阶段、CPAP 使用时长、夜间最低血氧、设备同步完整率等设备依赖目标时不可关闭 | 有绑定设备但关闭时必填原因 |
| 生活方式方案 | 默认建议纳入 1 条轻量指导 | 无不可关闭场景 | 不要求原因 |

模块卡片交互 SPEC：

```text
模块标题 + 状态标签
  必须模块：必填 / 已完成 / 待补充 / 有风险 / 已修改
  条件模块：纳入本方案开关 / 未纳入原因 / 已完成 / 待补充

摘要区
  展示当前配置摘要、系统推荐依据、关键最近数据

操作区
  必须模块：编辑
  条件模块：编辑、纳入本方案、重新纳入、查看关闭原因
```
...
医生点击"关闭纳入"时，弹窗展示关闭后果，帮助医生理解关闭的影响：
...
- 点击确认下发时，系统校验：方案基础信息、管理目标、至少 1 个关键测量项、核心预警规则、随访计划、患者指导、条件模块依赖关系。
- 多病共管时，系统按疾病分组展示目标和方案项，并提示重复待办合并结果。
- 医生从 0 创建时，如果没有选择模板，系统不自动生成治疗类内容，只提供测量、记录、随访和预警基础建议。

#### 15.7.4 管理方案交互开发 SPEC

管理方案页面交互、模块字段、状态按钮、下发校验、患者可见内容预览、前端 Mock Schema 和开发拆分已拆分至独立 SPEC：

[医生PC端-管理方案SPEC](./spec/医生PC端-管理方案SPEC.md)

主 PRD 本节仅保留管理方案业务定位、P0 范围、核心流程、模块规则、状态定义及与患者端/任务/预警/随访的关系。
...
- 阶段目标不能为空。
- 至少启用 1 个量化目标。
- 医生修改系统默认目标值时自动留痕记录修改内容。

#### 15.7.6 快速审核通道

系统推荐方案的大部分内容医生无需逐模块审核。P0 提供"快速审核"入口，帮助医生在 3 分钟内完成审核下发。
...
适用场景：
- 系统推荐方案医生整体认可，只需确认关键参数
- 已下发方案仅做少量正式调整时，快速确认后下发

不适用场景：
- 医生需要大幅修改方案内容 → 应进入逐模块编辑模式
...
| 状态 | 说明 |
| --- | --- |
| 草稿 | 医生编辑中或系统推荐待确认，患者不可见 |
| 已下发待患者确认 | 医生已确认下发，患者端可见，等待患者确认知晓 |
| 执行中 | 患者已确认知晓并开始执行 |
| 已停用 | 医生停用，不再执行，保留历史 |
| 已完成 | 阶段结束 |
| 已驳回 | 医生驳回系统推荐，仅系统草稿场景 |

按钮规则：

| 状态 | 主按钮 | 二级操作 |
| --- | --- | --- |
| 草稿（含系统推荐） | 审核/继续编辑 | 保存草稿、确认下发、驳回推荐（仅系统草稿）、删除草稿 |
| 已下发待患者确认 | 提醒患者确认 | 标记已知晓、撤回下发 |
| 执行中 | 查看方案 | 调整方案（原方案进入调整态）、停用方案、创建随访、结束本阶段 |
| 已停用 | 查看历史 | 复制为新方案 |
| 已完成 | 查看复盘 | 复制为新方案 |
| 已驳回 | 查看详情 | 复制为新方案 |
...
| 标签 | 展示/处理方式 |
| --- | --- |
| 患者未确认 | 已下发待患者确认 + "患者未确认"标签 |
| 患者有疑问 | 执行中 + "患者有疑问"标签，医生可发送建议或调整方案 |
| 患者无法执行 | 执行中 + "患者无法执行"标签，医生可发送建议、调整方案或停用方案 |
| 即将到期 | 执行中 + "即将到期"标签 |
| 待复盘 | 执行中 + "待复盘"标签，医生可结束本阶段或调整方案 |
| 调整版本 | 草稿 + "调整版本"标签 |
...
草稿 -> 已下发待患者确认: 医生确认下发
草稿 -> 已驳回: 医生驳回系统推荐
已下发待患者确认 -> 执行中: 患者确认已知晓并开始执行
已下发待患者确认 -> 草稿: 医生撤回下发
执行中 -> 草稿: 医生调整方案，原方案进入调整态，保留同一方案记录和版本轨迹
执行中 -> 已停用: 医生停用方案
执行中 -> 已完成: 医生结束本阶段
已驳回/已停用/已完成 -> 草稿: 复制为新方案
执行中 + 患者有疑问标签: 医生处理后去除标签、发送建议或调整方案
执行中 + 患者无法执行标签: 医生处理后去除标签、发送建议、调整方案或停用
执行中 + 待复盘标签: 医生复盘后继续执行、在原方案上调整或结束本阶段
```
...
| 患者反馈疑问 | 患者 | 方案保持执行中，标注"患者有疑问"标签，通知主责医生处理 |
| 患者反馈无法执行 | 患者 | 方案保持执行中，标注"患者无法执行"标签，通知主责医生处理 |
...
- 如果患者超过 48 小时未确认，医生端患者列表和方案管理列表标记“患者未确认”。
- 安全类医生建议、紧急预警和线下就医提示不等待患者确认方案后才展示，应下发即展示并要求患者确认已读。
...
| 我有疑问 | 方案保持执行中，添加“患者有疑问”标签，要求患者填写问题并通知主责医生 |
| 暂时无法执行 | 方案保持执行中，添加“患者无法执行”标签，要求患者选择原因并可补充说明 |

### 15.8.3 执行态目标与实际对比

方案进入执行中后，医生需要快速判断患者是否按方案执行、目标是否在接近。P0 在方案详情页增加"执行概览"区域。
...
- 支持预警后、方案复盘和医生临时创建随访。
- 支持患者端生成随访准备任务。
- 支持医生查看随访前摘要、填写结构化结论、生成下一步动作。
- 不做在线问诊、视频问诊、IM 医患聊天、复杂排班、多医生协同随访、复杂问卷库后台、批量随访和随访质控大屏。

产品边界：

- 随访是结构化慢病管理复盘，不是问诊服务。
- 随访可以生成医生建议、补充任务、复诊/转诊建议或管理方案调整，但不承载实时问诊聊天。

P0 目标：

- 判断患者是否按管理方案执行。
- 判断关键指标、症状、用药、设备数据是否改善或恶化。
- 判断是否需要继续当前方案、调整方案、补充数据、发医生建议、建议复诊/转诊或安排下次随访。

### 16.1 随访与管理方案关系

```mermaid
flowchart TD
  A["管理方案确认下发"] --> B{"是否配置随访计划"}
  B -->|是| C["生成计划内随访"]
  B -->|否| D["不生成计划内随访"]
  ...
  J --> K{"医生结论"}
  K -->|继续当前方案| L["保存结论，生成下次随访"]
  K -->|需短期处理| M["创建临时干预"]
  K -->|确需改长期方案| N["填写调整类型与原因后进入方案调整"]
  K -->|资料不足| O["生成补充记录/报告任务"]
  K -->|需要复诊/转诊| P["发送医生建议，创建追踪随访"]
  K -->|结束本阶段| Q["关闭本阶段随访计划"]
...
- 随访结束可调用方案编辑器生成新版本。
- 临时随访不一定改变方案，但必须记录结论。
- 每条计划内随访必须记录来源方案 ID 和方案版本，确保复盘的是对应版本的方案效果。

### 16.1.1 初始化管理方案的随访计划

初始化管理方案中的随访计划是方案的一部分，不要求医生从 0 单独创建。系统根据疾病、患者当前风险状态和方案周期自动带出默认随访规则；方案生效后，系统按规则生成第一条计划内随访。

P0 默认随访规则：

| 疾病/风险 | 首次随访时间 | 随访重点 | 患者准备材料 |
| --- | --- | --- | --- |
| 糖尿病低风险 | 方案下发后 4 周 | 血糖达标、记录完整度、用药执行 | 近 7 天血糖、用药打卡、饮食/运动备注 |
| 糖尿病中风险 | 1-2 周 | 异常时点、低/高血糖、用药依从性 | 近 7 天多时点血糖、低血糖症状、用药记录 |
| 糖尿病高风险 | 3-7 天 | 低血糖/持续高血糖风险 | 近 3-7 天血糖、症状、用药、饮食备注 |
| 慢阻肺低风险 | 4 周 | 症状稳定性、吸入药执行 | SpO2、呼吸频率、咳痰气促记录、吸入药打卡 |
...
危急/急性风险不生成普通计划内随访，默认生成 24-72 小时追踪随访，并提示医生优先处理线下就医、复诊或转诊建议。
...
| 状态 | 说明 |
| --- | --- |
| 待患者准备 | 已创建随访，等待患者完成准备材料 |
| 待医生处理 | 到达计划时间或准备材料已完成，等待医生随访 |
| 已逾期 | 超过计划时间未处理 |
| 已完成 | 医生已保存随访结论 |
| 已改期 | 原计划时间已调整，保留改期记录 |
| 已取消 | 医生取消，需记录原因 |
...
stateDiagram-v2
  ...
  pending_patient_prepare --> pending_doctor: 到达随访时间/准备完成
  pending_doctor --> completed: 医生完成随访
  pending_doctor --> rescheduled: 改期
  pending_doctor --> cancelled: 取消
  pending_doctor --> overdue: 超时未处理
  overdue --> completed: 补完成
  overdue --> rescheduled: 改期
  completed --> completed: 记录临时干预或正式调方案结果
```
...
筛选区
  时间：今日 / 本周 / 逾期 / 自定义
  随访类型：计划内 / 预警后 / 方案复盘 / 临时
  疾病：糖尿病 / 慢阻肺 / 睡眠呼吸障碍 / 高血压
  状态：待患者准备 / 待医生处理 / 已完成 / 已逾期
...
- P0 默认列表视图，不建设复杂日历排班；后续如需要再扩展日历视图。
- 顶部统计卡支持快捷筛选，点击“今日待随访/逾期随访/预警后随访/待患者准备”后自动筛选列表。
- 列表优先突出计划时间、准备完成度、关键异常摘要和“开始随访”按钮。
- 逾期和预警后随访使用状态色强调，但不使用“高风险”文案，避免与风险分层混淆。
- 准备完成度低于 50% 时，开始随访按钮旁提示“资料不足，可先发送补充准备提醒”。
...
- P0 使用右侧抽屉或中等宽度弹窗，不单独建设复杂创建页面。
- 从管理方案、预警、患者 360 进入时，患者、来源、关联对象自动带入。
- 表单底部固定操作区：取消、保存草稿、确认创建。
...
| 设备数据 | 正常同步、部分缺失、未同步 |
| 医生结论 | 继续方案、临时干预、调整方案、资料不足、复诊/转诊、结束本阶段 |
| 患者端建议 | 患者可见 |
| 医生内部备注 | 患者不可见 |
| 下次随访时间 | 可选；继续方案或调整方案后可由系统带出 |

视觉与交互：

- 左侧摘要偏阅读，使用卡片分组展示关键异常和完成度，不让医生在随访时重新翻所有记录。
- 中间记录区使用结构化单选、复选和短文本，避免只写长文本。
- 右侧下一步动作固定展示，医生选择结论后动态高亮推荐动作。
- 点击“创建临时干预”时，生成临时随访或临时任务，不改变当前长期方案。
- 点击“调整管理方案”时，必须先填写调整类型与调整原因，再进入方案编辑器，并把本次随访结论带入证据区。
- 点击“资料不足”时，系统生成补充记录、报告上传或评估量表任务。
...
| 指标未改善，加强管理 | 优先创建临时任务或临时随访；确需长期变化时再进入方案调整 |
| 出现高风险，需要复诊/转诊 | 生成医生建议、转诊提示和随访追踪 |
| 资料不足，需补充数据 | 生成患者端补充记录/报告待办 |
| 结束本阶段 | 关闭本阶段随访计划，当前方案进入已完成 |

规则：

- 医生完成随访时必须选择一个结论，不允许只保存空记录。
- 选择“继续当前方案”时，系统按原随访规则生成下一次计划内随访。
- 选择“临时干预”时，生成临时任务或临时随访并留痕。
- 选择“调整管理方案”相关结论时，必须填写调整类型与调整原因，并在原方案上进入调整态。
- 选择“资料不足”时，必须选择补充材料类型：指标记录、症状记录、用药记录、设备/睡眠报告、检查报告、评估量表。
- 选择“复诊/转诊”时，必须填写患者端可见说明，并可创建追踪随访。
- 取消或改期随访必须留痕，记录操作人、时间、原因和原计划时间。
...
- P0 必须支持从管理方案模板自动生成患者端待办，包括指标记录、症状记录、用药/治疗执行、设备绑定/同步、睡眠报告、随访准备、复测、确认知晓。
...
`报告上传` 和复杂量表类任务在 P0 不作为首页独立任务主形态重点暴露，可放入随访准备子项或记录页二级入口承载。

### 17.3 任务可编辑字段

医生在方案审核、随访创建、预警处理和医生建议中配置方案事项时，底层任务模型应支持以下字段：
...
- 方案下发后，先生成确认知晓任务；患者确认“已知晓并开始执行”后，再按计划生效时间生成周期性任务。
...
- 预警任务不能被患者跳过；逾期后仍保留医生可见状态。

医生建议生成待办：

- 医生发送医生建议时可勾选“生成患者端待办”。
- 医生建议待办可以是一次性，也可以转为短周期待办。
- 患者完成后医生端显示执行状态。
...
- `overdue` 不再作为独立主状态，而是 `pending` 的时间标签；前端展示为“已逾期”，底层仍是 `pending`。
- 暂不保留 `unable`，患者端所有任务都不提供“无法完成”操作。
- 暂不保留 `invalidated`；任务失效统一并入 `closed`，通过 `close_reason` 区分“方案版本调整 / 医生关闭 / 自动关闭”。
- 任务逾期后补完成，直接进入 `completed`，并记录补录时间与实际发生时间。
...
| `device_monitoring_plan` | 设备任务/睡眠报告/CPAP 任务 | 设备类型、是否推荐绑定、患者端说明 |
| `followup_rule` | 随访准备 | 首次随访时间、准备材料、问卷、报告、患者说明 |
| `alert_rules` | 复测任务/随访准备 | 触发条件、预警等级、患者动作、医生动作 |
| `patient_description` | 确认知晓 | 方案说明、风险提示、患者确认回执 |

P0 必须支持的任务生成结果：

| 方案模块 | P0 是否必须生成任务 | 说明 |
| --- | --- | --- |
| 管理目标 | 否 | 作为任务判断目标，不单独生成待办；但方案下发需生成确认知晓 |
...
医生建议统一作为医生下发给患者的结构化信息。页面文案统一使用“医生建议”，不使用“医嘱与指导”作为功能名称，避免患者误解为处方或正式院内医嘱。

医生建议需要区分严肃程度和执行要求：

| 类型 | 定义 | 示例 |
| --- | --- | --- |
| 复测建议 | 要求患者在指定时间补充或复测指标，并形成记录留痕 | 复测血糖、连续记录晨起血氧 |
| 执行建议 | 围绕既有方案、既有处方或设备使用给出的执行提醒 | 按当前方案使用吸入药、完成 CPAP 佩戴记录 |
| 就医/转诊建议 | 建议患者线下复诊、专科评估或必要时及时就医 | 尽快线下复诊、预约呼吸专科进一步评估 |
| 健康指导 | 健康教育、生活方式、设备使用、记录说明等管理建议 | 控盐、运动、睡眠卫生、设备佩戴说明 |
...
| `related_plan_version` | number | 关联方案版本 |
| `related_alert_id` | string | 关联预警 |
| `scheduled_at` | datetime | 计划随访时间 |
| `original_scheduled_at` | datetime | 改期前计划时间 |
| `method` | string | mini_program/phone/offline/doctor_record |
| `prepare_items` | json | 准备材料 |
| `prepare_completion` | object | 准备材料完成度和缺失项 |
| `patient_instruction` | text | 患者端说明 |
| `status` | string | pending_patient_prepare/pending_doctor/overdue/completed/rescheduled/cancelled |
| `reschedule_reason` | text | 改期原因 |
| `cancel_reason` | text | 取消原因 |
| `next_followup_rule` | object | 下一次随访生成规则 |
| `created_at` | datetime | 创建时间 |
...
| `purpose` | string | planned/alert/plan_review/manual/referral |
| `prepare_status` | string | completed/partial/missing |
| `metric_judgement` | string | improved/stable/worse/insufficient_data |
| `symptom_judgement` | string | no_change/improved/worse |
| `treatment_adherence` | string | good/fair/poor/unknown |
| `device_data_status` | string | normal/partial_missing/not_synced |
| `doctor_conclusion` | string | 随访结论 |
| `doctor_advice` | text | 患者可见建议 |
| `internal_note` | text | 医生内部备注 |
| `plan_action` | string | continue/current_adjust/need_more_data/referral/end_stage/no_change |
| `adjusted_plan_id` | string | 进入调整态的原方案 ID |
| `generated_task_ids` | json | 随访后生成的补充任务、复测任务或确认知晓任务 |
| `next_followup_at` | datetime | 下次随访时间 |
| `completed_at` | datetime | 完成时间 |

### 21.12 patient_timeline_event

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | string | 时间轴事件 ID |
| `patient_id` | string | 患者 ID |
| `event_type` | string | screening/record/device/alert/plan/followup/order_guidance/report |
| `event_subtype` | string | 如 plan_published/followup_completed/alert_resolved |
| `event_time` | datetime | 事件发生时间 |
| `title` | string | 事件标题 |
| `summary` | text | 事件摘要 |
| `source_type` | string | 来源对象类型 |
| `source_id` | string | 来源对象 ID |
| `related_plan_id` | string | 关联方案 ID |
| `related_plan_version` | number | 关联方案版本 |
| `related_followup_id` | string | 关联随访 ID |
| `related_alert_id` | string | 关联预警 ID |
| `operator_type` | string | system/patient/doctor/device |
| `operator_id` | string | 操作人或设备 ID |
| `visibility` | string | doctor_only/patient_visible/internal |
| `status` | string | active/invalidated/deleted |
| `payload` | object | 事件扩展信息，如结论、后续动作、关键指标 |
| `created_at` | datetime | 创建时间 |

生成规则：

- 管理方案、随访、预警、医生建议、设备报告等关键事件写入时间轴。
- 时间轴事件只读展示，不作为原始业务数据的唯一存储。
- 原始对象被更新后，时间轴事件保留当时摘要，并通过详情入口查看最新对象。
- 标记无效的记录不从时间轴删除，状态改为 invalidated。
...
| `priority` | string | normal/important/urgent |
| `status` | string | pending/completed/closed |
| `allow_backfill` | boolean | 是否允许补记 |
| `is_overdue` | boolean | 是否已逾期；仅作为 pending 的展示标签 |
...
| `doctor_note` | text | 医生内部备注 |
| `closed_at` | datetime | 任务关闭时间 |
| `close_reason` | text | 任务关闭原因，例如方案版本调整 |
| `created_at` | datetime | 创建时间 |

### 21.15 task_execution_log
...
| `patient_id` | string | 患者 ID |
| `action` | string | complete/backfill/overdue/close/reopen |
...
| 接口 | 方法 | 说明 |
| --- | --- | --- |
| `/api/doctor/patients` | GET | 患者列表，包含顶部统计卡、快捷筛选和待处理事项摘要 |
...
- 主责医生可查看患者授权范围内数据，并负责当前管理方案、预警、随访和医生建议闭环。
