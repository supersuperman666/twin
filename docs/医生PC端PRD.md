...
| 随访结束 | 医生选择“需要调整方案” | 原方案进入调整态（草稿 + 调整版本） |
| 预警处理 | 医生选择“调整管理方案” | 原方案进入调整态（草稿 + 调整版本） |
...
- 随访结束或预警处理后，医生选择“需要调整方案/调整管理方案”时，当前执行方案进入调整态，保留同一方案记录、来源事件和版本轨迹；若该方案已处于调整态，则提示医生继续编辑、合并新建议或覆盖未确认推荐项。
...
- 点击“调整管理方案”时，当前执行方案进入调整态，并把本次随访结论带入方案编辑器证据区。
...
- 选择“调整管理方案”相关结论时，随访状态进入已转方案调整，并在原方案上进入调整态。
