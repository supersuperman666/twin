...
统计概览
  全部患者  重点关注  疾病风险待确认  待处理预警  待确认方案  今日待随访  数据缺失

左侧筛选区
  默认展开；支持收起为窄栏
  重点关注、风险状态、疾病风险、确诊疾病、待处理事项、方案状态、随访状态、数据状态、医患关系
...
- 左侧筛选区默认宽度 220-240px；收起后宽度 40-48px，仅保留筛选 icon 和已选数量徽标。
- 筛选区收起后，患者列表横向空间自动扩大；再次点击筛选 icon 恢复展开。
...
- 筛选区收起时，已选条件仍在列表上方展示，避免医生不知道当前处于筛选态。
- 筛选区收起按钮固定在筛选面板右上角；收起后窄栏展示筛选 icon 和已选数量。
