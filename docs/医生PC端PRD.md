...
| 重点关注 | 全部、仅看重点关注 |
| 风险状态 | 稳定、需关注、需干预 |
| 疾病风险 | 全部、糖尿病风险、慢阻肺风险、睡眠呼吸暂停风险、高血压风险、待医生确认 |
| 确诊疾病 | 糖尿病、慢阻肺、睡眠呼吸暂停、高血压、多病共管 |
| 待处理事项 | 全部、有预警、待确认方案、今日待随访、数据缺失 |
| 方案状态 | 无方案、待医生确认、待患者知晓、执行中、待调整 |
...
| 去确认方案 | 有待医生确认方案 | 跳转方案管理并定位该患者/方案 |
...
- P0 必须保留预警到复测、随访、医生建议、管理方案调整入口的闭环。
...
右侧审核区
  ...
  必填项完成度
  患者可见内容预览
  操作按钮：保存草稿、患者可见内容预览、确认下发、修改后下发、驳回推荐
```
...
flowchart TD
  ...
  K -->|需要调整方案| M["基于当前方案生成新版本草稿"]
  K -->|资料不足| N["生成补充记录/报告任务"]
  K -->|需要复诊/转诊| O["发送医生建议，创建追踪随访"]
  K -->|结束本阶段| P["关闭本阶段随访计划"]
...
初始化管理方案中的随访计划是方案的一部分，不要求医生从 0 单独创建。系统根据疾病、风险等级、方案周期自动带出默认随访规则；方案生效后，系统按规则生成第一条计划内随访。
...
stateDiagram-v2
  ...
  completed --> plan_adjustment: 选择调整方案
```
...
- 点击“调整管理方案”时，基于当前执行方案生成新版本草稿，并把本次随访结论带入方案编辑器证据区。
...
| 结束本阶段 | 关闭本阶段随访计划，当前方案进入待复盘或已完成 |

规则：

- 医生完成随访时必须选择一个结论，不允许只保存空记录。
- 选择“继续当前方案”时，系统按原随访规则生成下一次计划内随访。
- 选择“调整管理方案”相关结论时，随访状态进入已转方案调整，并生成新版本草稿。
...
| `status` | string | pending_patient_prepare/pending_doctor/overdue/completed/rescheduled/cancelled/plan_adjustment |
...
| `plan_action` | string | continue/current_adjust/new_plan/need_more_data/referral/end_stage/no_change |
| `new_plan_id` | string | 新方案 ID |
| `generated_task_ids` | json | 随访后生成的补充任务、复测任务或确认阅读任务 |
