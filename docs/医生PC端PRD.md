...
- 专科医生处理紧急预警、复杂治疗方案、疑似急性加重、疑似治疗不敏感和诊断辅助。
...
扫码绑定用于建立医生与患者的服务关系。当前版本只支持“医生生成二维码，患者扫码确认授权”这一条主流程；暂不纳入后台/机构分配患者、患者扫码机构码、服务团队绑定。绑定生效后，医生端可查看患者授权范围内的数据，患者端可看到绑定医生、医嘱与指导、随访计划和方案待办。
...
| 用药方案 | 用药提醒、吸入药、漏服记录、用药备注 | 是，诊疗敏感内容按医生确认后展示 |
...
| 管理目标 | structured | 是 | 包含阶段目标、量化目标、目标周期、达成判断口径 |
| 指标测量方案 | plan builder | 是 | 测量频率、时间点、目标范围、录入方式 |
| 症状记录方案 | plan builder | 可选 | 症状项、严重程度、持续时间、是否关联指标 |
| 用药方案 | plan builder | 可选 | 药品、剂量、频次、服用时间、漏服记录 |
| 设备监测方案 | plan builder | 可选 | 设备类型、同步要求、缺失提醒、关键报告项 |
| 生活方式方案 | checklist | 可选 | 饮食、运动、戒烟、睡眠、肺康复等 |
| 预警规则 | rule list | 是 | 可启用/停用/调整阈值 |
| 随访计划 | followup scheduler | 是 | 自动生成计划内随访 |
...
- 系统推荐方案不得直接下发给患者。
- 已下发方案不可直接覆盖，只能生成新版本。
- 患者端可确认已知晓、完成今日待完成事项、填写备注，但不可修改医生确认的目标和治疗方案。
