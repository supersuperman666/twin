...
| --- | --- | ---: | --- |
| 1 | 基础信息 | 3 | 出生年份、性别、身高 |
| 2 | 疾病史 | 2 + N | 本人疾病史、已选慢性疾病逐项确诊年限、父母家族史 |
...
| 4.1 | `diseaseDurations` | 已确认慢性疾病确诊年限 | 条件单选组 | 每个已选慢性疾病必填 1 个年限：1 年以内、1-3 年、3-5 年、5-10 年、10 年以上 | 不扣分，沉淀健康档案病程锚点 |
...
- `disease` 选择任一慢性疾病后，需在当前步骤按所选疾病逐项展示“确诊已有多少年”，每个疾病年限均为必填；选择“无”时不展示年限题。
- `diseaseDurations` 按疾病分别保存，例如同时选择糖尿病和高血压时，需分别保存两项确诊年限，不能共用一个年限答案。
...
| 70-84 | 中风险 | 存在部分健康风险 |
| 50-69 | 高风险 | 存在较明显健康风险 |
| 0-49 | 危急/高风险 | 风险较高，建议尽快联系医生 |

说明：CSV 中未给出风险等级阈值，以上阈值为产品 MVP 建议，需医学负责人确认。患者端展示“健康风险分”，分值越高表示近期状态越稳定；医生端同步展示分值、风险等级、扣分证据和规则版本。

### 6.1.1 筛查评分与日常健康风险分关系

- 首次筛查评分作为用户初始健康风险分。
- 后续每日健康风险分在筛查基础上，叠加指标记录、睡眠报告、症状、用药、设备同步和数据完整度。
- 若用户当日无任何新增健康数据，则沿用最近一次有效评分，并标记“数据未更新”。
- 若出现危急预警，例如严重低血糖、持续低氧或症状明显加重，即使基础筛查分较高，也需要按预警规则限制当日总分上限。
- 历史评分必须保存快照，避免后续补录或规则调整覆盖历史结果。
...
- 风险等级阈值是否采用 `85-100 低风险、70-84 中风险、50-69 高风险、0-49 危急/高风险`。
