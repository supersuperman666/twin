...
MVP 聚焦糖尿病、慢阻肺、睡眠呼吸障碍风险三类核心场景，并纳入高血压作为常见共病管理场景，优先跑通“筛查建档 - 生成管理方案 - 日常记录 - 风险预警 - 医生干预 - 随访复盘”的闭环。
...
- 展示当前方案名称、适用疾病、阶段目标、方案周期和医生信息。
- 按天生成任务，合并同时间段任务，例如晨起血压和空腹血糖。
- 展示医生确认的指标目标和患者可理解的预警提示。
- 支持患者确认已知晓、执行任务、填写备注和查看历史方案。
...
- 等级文案，例如“需要关注”。
- 分值变化，例如“较昨日 -4”。
- 主要原因，最多展示 3 条，例如“昨夜低氧时间增加”“今日空腹血糖偏高”“一次用药未打卡”。
...
- 首页 `今日待办` 或风险提示卡片 -> `pages/plan/index`
- Tab `方案` -> `pages/plan/index`
- `pages/plan/index` 的 `今日任务` -> 进入具体执行页
...
| 任务类型 | 页面入口 | 建议跳转页面 |
| --- | --- | --- |
| 指标记录 | 方案页今日任务 / 首页待办 / 记录页快捷入口 | `/pages/record/form/index?type=glucose`、`/pages/record/form/index?type=blood_pressure`、`/pages/record/form/index?type=oxygen` |
| 睡眠报告 | 方案页今日任务 / 首页风险提示 / 记录页 | `/pages/sleep/report/index` |
| 用药/治疗执行 | 方案页今日任务 / 首页用药建议 / 记录页 | `/pages/medication/detail/index` |
| 症状评估 | 方案页今日任务 / 风险提示后补充症状 / 记录页 | `/pages/symptom/detail/index` |
| 设备任务 | 方案页今日任务 / 我的设备 / 记录页设备入口 | `/pages/device/index/index` |
| 随访准备 | 方案页 `随访计划` Tab | `/pages/plan/index` 内随访页签承载 |
...
- 方案页 `今日任务`：只放今天最该完成的任务，是主执行入口
- 方案页 `方案详情`：解释为什么要做，不承担高频执行
- 方案页 `随访计划`：展示下次随访和准备材料
