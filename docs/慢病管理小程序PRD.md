...
- 完成任务，完成后写入对应记录或执行日志。
- 补记任务，需保留实际发生时间和补记时间。
- 反馈无法完成原因；任务不会被跳过或消失，仍按待完成或逾期处理。
...
改善加分用于鼓励患者持续管理，仅用于恢复部分扣分，不允许抵消危急异常。例如连续完成记录、用药打卡、复测正常可增加 1-5 分；当日存在高风险预警未处理时，最高分不得超过 69 分；存在危急预警时，最高分不得超过 49 分。
...
| 关键任务连续未完成 | 近期关键记录缺失 | 补记录或反馈无法完成原因 |
...
| `/api/tasks/{id}/unable` | POST | 反馈任务无法完成原因 |
...
| 用药 | 按计划服用 | 单次未完成 | 关键药物连续 2 次未完成或反馈无法完成 |
...
| 连续 3 天空腹血糖高于目标 | 10-15 | 同时触发高风险预警 |
| 血糖 < 3.9 mmol/L | 10-20 | 需结合症状和是否需要协助 |
| 血糖 < 3.0 mmol/L | 25-40 | 触发危急预警，总分上限 49 |
| 睡眠期间 SpO2 < 90% 累计 >= 12 分钟 | 12-20 | 同步睡眠/呼吸风险 |
| SpO2 < 88% 或伴胸闷气促 | 25-40 | 触发危急/高风险预警 |
| AHI >= 15 | 8-12 | 中度睡眠呼吸风险 |
| AHI >= 30 | 15-25 | 高风险，建议医生确认 |
