...
function renderAdvancedFilters() {
  return `<section class="advanced-filter-panel panel">
    ...
    <div class="advanced-filter-grid">
      ...
      ${filterBlock("待处理事项", "todo", ["全部", "有预警", "今日待随访", "数据缺失"])}
...
function patientRow(patient) {
  ...
  const hasMore = pendingAlerts.length || pendingFollowups.length || hasPendingDiseaseRisk(patient) || hasDataIssue(patient);
  ...
  return `<tr>
    ...
    <td><small>${nextFollow ? `${nextFollow.dueAt} ${nextFollow.status}` : "暂无随访"}</small></td>
    <td>
...
function renderOverview(patient) {
  ...
  return `
    <div class="overview-grid">
      ...
      <section class="panel">
        <div class="panel-hd"><strong>当前执行方案</strong></div>
        <div class="info-block"><h3>暂无方案</h3><p>暂无当前执行方案</p></div>
      </section>
    </div>
